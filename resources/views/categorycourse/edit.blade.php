...
<body>
    <form action="{{ route('coursecategory.update') }}"  enctype='multipart/form-data' method="POST">
        ...
        <div>
            ...
            <input type="text" id="title" name="title"  value="{{$coursecategory->title}}" required>
        </div>
        <div>
            <label for="description">Description:</label>
            <input type="text" id="description" name="description"  value="{{$coursecategory->description}}" required>
        </div>
        <div>
            </div>
        <select name="parent_id">
                ...
                <option value="{{$cat->id}}" @if($cat->id==$coursecategory->parent_id)selected @endif >{{$cat->title}}</option>
                @endforeach
        </select>
            </div>
             <div>
            <label for="show_in_home">Showhome:</label>
            <input type="checkbox" name="homes" value="1" @if($coursecategory->show_in_home) {{ "checked" }}  @endif>
        </div>
