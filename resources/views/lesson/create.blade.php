<form action="{{ route('lesson_store') }}" method="POST" enctype="multipart/form-data" style="max-width:600px; margin:auto;">
    @csrf
    ...
    <div style="margin-bottom:15px;">
        ...
        <select name="course_id" id="course_id" style="width:100%; padding:6px;">
            ...
            @foreach($courses as $cour)
                <option value="{{ $cour->id }}" @if(isset($course) && $course->id == $cour->id) selected @endif>{{ $cour->title }}</option>
            @endforeach
        </select>
    </div>
    ...
    <div style="margin-bottom:15px;">
        ...
        <select name="season_id" id="season_id" style="width:100%; padding:6px;">
            ...
            @foreach($seasons as $seas)
                <option value="{{ $seas->id }}" @if(isset($season) && $season->id == $seas->id) selected @endif>{{ $seas->title }}</option>
            @endforeach
        </select>
    </div>
