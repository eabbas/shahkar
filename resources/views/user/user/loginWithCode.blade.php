...
<head>
    ...
    <title>شاهکار | ورود با کد تایید</title>
</head>
...
<body>
    ...
    <main class="w-full lg:h-dvh flex flex-row items-center p-4">
        <div class="w-full lg:w-1/2 lg:h-full p-4 lg:p-10 flex flex-col items-center gap-10">
            <h2 class="text-2xl font-bold text-gray-800 text-center mt-10">ورود با کد تایید</h2>
            ...
            <form action="{{ route('user.checkUserWithCode') }}" method="post" class="w-9/12 flex flex-col gap-4"
                id="setPassForm">
                @csrf
                <div class="w-full">
                    <input type="number"
                ...
                <div class="w-full flex flex-row justify-between items-center gap-2">
                    ...
                    <a href="{{ route('user.login') }}"
                        class="text-sm flex flex-row justify-start items-center gap-2 text-gray-400">ورود با رمز
                        عبور</a>
                </div>
    ...
    <script>
        ...
        function sendCode() {
            ...
            if (phoneNumber.value == "") {
                showMessage('open')
                element.innerHTML = `
                                <span class="text-red-500"> شماره تلفن را وارد کنید !</span>
                            `
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        if (data) {
                            ...
                            showMessage('open')
                            element.innerHTML = `
                                <span class="text-red-500">کاربر قبلا با این شماره ثبت نام نکرده است!</span>
                            `
                            message.children[0].appendChild(element)
                            setTimeout(() => {
                                showMessage('close')
        ...
        let setPassForm = document.getElementById('setPassForm')
                ...
                $.ajax({
                    ...
                    success: function(user) {
                        ...
                        if (!user.validate) {
                            showMessage('open')
                            element.innerHTML = `
                                <span class="text-red-500">شما قبلا با این شماره ثبت نام نکرده اید!</span>
                            `
                            message.children[0].appendChild(element)
                            setTimeout(() => {
                                showMessage('close')
