<form action="{{ route('lesson_update' , $lesson->id) }}" method="POST" enctype="multipart/form-data" style="max-width:600px; margin:auto;">
    @csrf
    ...
    <div style="margin-bottom:15px;">
        ...
        <input type="text" name="title" id="title" value="{{ $lesson->title }}" style="width:100%; padding:6px;">
    </div>
    ...
    <div style="margin-bottom:15px;">
        ...
        <input type="number" name="price" id="price" value="{{ $lesson->price }}" style="width:100%; padding:6px;">
    </div>

    <div style="margin-bottom:15px;">
        <label for="order">ترتیب نمایش:</label><br>
        <input type="number" name="order" id="order" value="{{ $lesson->order }}" style="width:100%; padding:6px;">
    </div>

    <div style="margin-bottom:15px;">
        <label for="duration">مدت زمان درس:</label><br>
        <input type="text" name="duration" id="duration" value="{{ $lesson->duration }}" style="width:100%; padding:6px;">
    </div>

    <div style="margin-bottom:15px;">
        <label>
            <input type="checkbox" name="free" value="1" {{ $lesson->free ? 'checked' : '' }}> این درس رایگان است
        </label>
    </div>
