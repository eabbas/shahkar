           ...
           <div style="display:flex; flex-wrap:wrap; gap:20px;">
               @foreach ($lesson->attachments as $attachment)
                   ...
                   <div style="border:1px solid #ccc; padding:10px; border-radius:5px; width:220px; text-align:center;">
                       ...
                       {{-- آپدیت --}}
                       <form action="{{ route('attachment_update', $attachment->id) }}" method="POST"
                           enctype="multipart/form-data" style="margin-top:10px;">
                           @csrf
                           <input type="file" name="file" required style="margin-bottom:5px;">
                           <button type="submit"
                               style="padding:4px 8px; background:#ffc107; border:none; border-radius:4px;">
                               آپدیت
                           </button>
                       </form>

                       {{-- حذف --}}
                       <a href="{{ route('attachment_delete', $attachment->id) }}"
                           style="display:inline-block; margin-top:5px; padding:4px 8px; background:#dc3545; color:white; border-radius:4px;">
                           حذف
                       </a>
