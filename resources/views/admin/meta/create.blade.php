...
@section('title', 'شاهکار | متا ها')
@section('content')
    <h2 class="text-3xl text-center font-bold py-10 text-[#425A8B]">فرم ایجاد متا</h2>
    <div class="lg:w-2/3 w-full mx-auto border border-[#D5DFE4] rounded-[10px] text-[#425A8B] p-5 bg-white">
        <form action="{{ route('meta_store') }}" method="post">
            @csrf
            <div class="w-full flex flex-col">
                ...
                <input type="text" class="outline-none pr-5 py-3 bg-[#F9F9F9] rounded-[12px] focus:bg-[#f1f1f4]"
                    name="title" id="title" placeholder="عنوان متا را وارد کنید" required>
            </div>
