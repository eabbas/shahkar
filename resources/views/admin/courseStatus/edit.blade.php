    ...
    @section('title')
        شاهکار | ویرایش {{ $status->title }}
    @endsection
    @section('content')
        <h2 class="text-3xl text-center font-bold py-10 text-[#425A8B]">{{ $status->title }}</h2>
            ...
            <form action="{{ route('status.store') }}" method="post">
                ...
                <div class="w-full flex flex-col">
                    ...
                    <input type="text" class="outline-none pr-5 py-3 bg-[#F9F9F9] rounded-[12px] focus:bg-[#f1f1f4]"
                        name="title" id="title" placeholder="عنوان را وارد کنید" required value="{{ $status->title }}">
                </div>
