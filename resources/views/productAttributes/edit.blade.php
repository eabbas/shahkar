...
<body>
   <div class="w-full h-svh bg-[url('https://keenthemes.com/static/metronic/tailwind/dist/assets/media/images/2600x1200/bg-10.png')] bg-cover bg-no-repeat">
      <h2 class="text-3xl text-center font-bold py-5 text-[#425A8B]">فرم ویرایش ویژگی های محصول</h2>
      <div class="w-2/3 mx-auto border border-[#D5DFE4] rounded-[10px] text-[#425A8B] p-5 bg-white">
         <form action="{{route('productAttribute-update')}}" method="post">
            @csrf
            <input type="hidden" name="productAttribute_id" value="{{$productAttribute['id']}}">
            <div class="flex flex-col gap-3">
               ...
               <select name="product_id" id="categoroy_id" class="md:w-1/2 bg-[#F9F9F9] py-3 pr-5 rounded-[10px]" required>
                  ...
                  <option value="{{$product->id}}" @if($product->id == $productAttribute->product_id) selected @endif>{{$product->title}}</option>
                  @endforeach
               </select>
            </div>
            <div class="mt-4 w-full flex flex-col">
               ...
               <input type="text" class="outline-none pr-5 py-3 bg-[#F9F9F9] rounded-[12px] focus:bg-[#f1f1f4]" name="attribute_key" id="attribute_key" placeholder="نام ویژگی محصول را وارد کنید" value="{{$productAttribute['attribute_key']}}" required>
            </div>
            <div class="mt-4 w-full flex flex-col">
               ...
               <input type="text" class="outline-none pr-5 py-3 bg-[#F9F9F9] rounded-[12px] focus:bg-[#f1f1f4]" name="attribute_value" id="attribute_value" placeholder="مقدار ویژگی محصول را وارد کنید" value="{{$productAttribute['attribute_value']}}" required>
            </div>
